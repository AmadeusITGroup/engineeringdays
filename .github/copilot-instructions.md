...
- OpenFeedback: `generate_openfeedback.py` (if applicable); `openfeedback.json` is generated by the deploy build and not committed

## GitHub Actions
- `deploy.yml` — Auto-deploys to GitHub Pages on push to `main`; runs `scripts/ci/build.py` (regenerate OpenFeedback data + validate JSON) before publishing
